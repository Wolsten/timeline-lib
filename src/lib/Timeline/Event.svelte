<script>
	...
	function handleClick() {
		...
		if (options.selectedEvent === false || options.selectedEvent.index !== event.index) {
			...
			console.log(clickMs, lastClickedMs, clickMs - lastClickedMs, options.selectedEvent);
			if (options.readonly === false && lastClickedMs && clickMs - lastClickedMs < 500) {
